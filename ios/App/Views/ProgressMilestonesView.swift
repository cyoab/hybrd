    ...
    VStack(alignment: .leading, spacing: 16) {
      HStack {
        VStack(alignment: .leading, spacing: 4) {
          Text(L10n.text("Your milestones")).font(.title3.weight(.semibold))
          Text(L10n.text("\(milestones.filter(\.earned).count) of \(milestones.count) unlocked · all time"))
            .font(.caption).foregroundStyle(HybrdStyle.muted)
        }
        Spacer(minLength: 8)
        if !showAll { Button(L10n.text("See all")) { showingCollection = true }.font(.subheadline.weight(.medium)) }
      }
        ...
        ForEach(showAll ? milestones : featured) { milestone in
          Button { selected = milestone } label: {
            VStack(spacing: 10) {
              ...
              Text(milestone.earned ? L10n.text("Unlocked") : milestone.kind.progressLabel(milestone.value, units: units))
          ...
          .accessibilityValue(milestone.earned ? L10n.text("Unlocked") : milestone.kind.progressLabel(milestone.value, units: units))
          .accessibilityHint(L10n.text("Shows the milestone and how it is earned"))
        }
      }
    }
  ...
  var body: some View {
    NavigationStack {
      ScrollView { ProgressMilestonesView(milestones: milestones, showAll: true).padding(20).frame(maxWidth: 720).frame(maxWidth: .infinity) }
        .background(HybrdStyle.background).navigationTitle(L10n.text("Milestones")).navigationBarTitleDisplayMode(.inline)
