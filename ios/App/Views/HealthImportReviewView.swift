...
struct HealthImportReviewView: View {
  ...
  var body: some View {
    NavigationStack {
      Form {
        Section {
          Text(L10n.text("Choose what to fill in. Existing entries are kept unless you select a replacement."))
            .font(.subheadline)
          if let birth = values.birthDate {
            Toggle(isOn: $useBirth) {
              LabeledContent(L10n.text("Date of birth"), value: birth.formatted(date: .abbreviated, time: .omitted))
            }
          }
          if let weight = values.weight {
            Toggle(isOn: $useWeight) { measurement(L10n.text("Weight"), weight, unit: editor.units.weight.symbol, displayedValue: editor.units.weight.value(fromKilograms: weight.value)) }
          }
          if let height = values.height {
            Toggle(isOn: $useHeight) { measurement(L10n.text("Height"), height, unit: "cm") }
          }
        } header: { Text(L10n.text("Available from Apple Health")) } footer: {
          Text(L10n.text("Missing fields may not be stored in Health or may not be shared. You can edit imported values before saving your profile."))
        }
      }
      .navigationTitle(L10n.text("Review Health import")).navigationBarTitleDisplayMode(.inline)
      .toolbar {
  ...
  private func measurement(_ title: String, _ value: HealthMeasurement, unit: String, displayedValue: Double? = nil) -> some View {
    VStack(alignment: .leading, spacing: 5) {
      Text(L10n.text("\(title) · \((displayedValue ?? value.value).formatted(.number.precision(.fractionLength(0...1)))) \(unit)"))
      Text(L10n.text("Measured \(value.date.formatted(date: .abbreviated, time: .omitted))")).font(.caption).foregroundStyle(.secondary)
    }
  }
}
