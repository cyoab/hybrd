  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 26) {
      ...
      LazyVGrid(columns: columns, alignment: .leading, spacing: 24) {
        metric("DISTANCE", units.distanceNumber(run.meters, decimals: 2), units.distance.symbol, "point.topleft.down.to.point.bottomright.curvepath")
        metric("ACTIVE TIME", RunRecording.clock(run.seconds(at: now)), "", "timer")
        metric("AVERAGE PACE", units.paceNumber(run.meters >= 10 ? run.seconds(at: now) * 1_000 / run.meters : nil), "/" + units.distance.symbol, "speedometer")
