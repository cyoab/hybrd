...
/// Timed-only plans use duration widths; mixed plans use equal steps without inventing pace.
struct WatchRunSequenceView: View {
  ...
  var body: some View {
    Canvas { context, size in
      let mixed = steps.contains { $0.segment.targets?.distanceM != nil }
      let visible = steps.filter { $0.seconds > 0 || $0.segment.targets?.distanceM != nil }
      let total = Double(max(1, mixed ? visible.count : visible.reduce(0) { $0 + $1.seconds }))
      var x: CGFloat = 0
      for step in visible {
        let width = size.width * Double(mixed ? 1 : step.seconds) / total
