...
## Watch workout cards and horizontal run pages

Reviewed [Runna's current/next workout view](https://mobbin.com/screens/2bcd311f-ae9f-4365-8f9a-7ca43544e6ee) and [Runna's completed-step metrics and controls](https://mobbin.com/screens/95fa71a4-3911-48a2-90a0-70e4f4a95aed) through Mobbin. These are iPhone references, adapted to the Watch rather than represented as Watch screenshots. Apple's [watchOS design guidance](https://developer.apple.com/design/human-interface-guidelines/designing-for-watchos) informed the glanceable metric hierarchy and short navigation depth.

The list pairs existing original shoe/dumbbell artwork with category-tinted cards and prescription graphics. Runs use time-proportional interval bars, colored by zone; strength shows one mark per exercise with its total prescribed set count. Illustrations and redundant charts are hidden from VoiceOver, while each card combines its readable workout labels.

The native horizontal pager opens on center metrics, with controls to the left and upcoming guidance to the right. Pace leads, then measured HR/zone, a five-segment zone infographic, active time and distance. Current zone uses a white outline/dot; the prescribed target is underlined and explicitly captioned with personal BPM limits when available. Pause and missing readings clear the live marker. Large pause/play and stop/finish controls have full tappable labels, finish confirmation, and a separate discard confirmation. Upcoming guidance shows the next prescription and repeat count, followed by the full workout description/sequence. On resume, confirmed native workout state returns the pager to metrics. Always On retains the existing reduced refresh cadence.

Both targets build and core/regression checks pass. Compact and larger offscreen renders were inspected, including no HR/GPS, missing personal zones and pause. Native simulator screenshots and gesture verification were blocked by Bitrig's simulator capture/control behavior; see `WORKOUT-RECORDING.md` for the actual app crash diagnosis and distinct validation limits.

## Independent measurement units

Athlete profile now has a Units section directly above Appearance. Two native segmented pickers select kilograms/pounds for lifting, body weight and strength PRs, and kilometers/miles for endurance distance and pace. Violet and Terra rows match the profile's strength/run colors. The choices preview within the profile and its PR/import/starter-review sheets, then apply across the app when Save profile succeeds. Invalid pending weight or weekly-distance entries must be corrected before switching their unit, avoiding silent reinterpretation of unfinished input.

Plans, session cards, goals/weekly rhythm, actual strength entry and previous sets, manual runs, live pace/distance, result summaries, Progress totals/trends/comparisons/milestones, coach totals and the Watch all use the selected units. Race categories such as 5K and Half marathon remain event names. Height remains in centimeters. Converted text is rounded for readability; unchanged form fields retain their precise original canonical values. Repeatedly toggling units does not change PRs, actuals or the training baseline.

Validation: conversion and legacy-decoding checks pass, including mixed preferences, comma-decimal input, range validation, 100 round trips through profile unit switching, manual result entry, pace/Progress conversion and kilometer/mile splits. An isolated in-memory test of the actual TrainingStore confirmed profile persistence, Watch snapshot publication and byte-equivalent unchanged plan snapshots/results. Both Apple targets build. Offscreen renders covered the light/dark settings, compact strength entry, imperial phone live/review screens and Watch metrics. The iPhone simulator reported it does not support simulator state, so native picker/keyboard/VoiceOver interaction is not claimed as verified.

## Account entry refresh — September 24, 2026

The email sign-in sheet now uses the existing shoe/dumbbell artwork, full-bleed adaptive color wash, compact wordmark toolbar, persistent input labels, and one primary email action. Code verification has a mint envelope illustration and one native six-digit field, preserving AutoFill, paste, leading zeroes, resend cooldowns, error handling and active-workout account restrictions. Connection settings are available from the labelled toolbar gear; backend domains are no longer foreground content.

Mobbin references inspected: [Mimo](https://mobbin.com/screens/e855a5d0-c5b0-4017-90fd-bce3c7cbd62a) for a friendly illustration and focused action hierarchy, [Anything](https://mobbin.com/screens/cf34fc72-82c1-4563-b773-92c39b8bd33c) for stacked provider buttons, and [Jomo](https://mobbin.com/screens/923e898f-14c9-4145-aabd-831f94955cec) for a clearly labelled, single-field verification step. Reference screenshots are not bundled.

Apple and Google buttons are explicitly marked coming soon and only present explanatory alerts. No OAuth, SDK, network calls or account creation occurs on either button. They use equal-sized white surfaces in both themes; the Google mark is the unmodified [official Google identity asset](https://developers.google.com/static/identity/images/g-logo.png), following the [Google sign-in branding guidance](https://developers.google.com/identity/branding-guidelines). Apple uses the unmodified `apple.logo` symbol solely for Sign in with Apple. All new copy is localized in English, Spanish, Brazilian Portuguese and French.
