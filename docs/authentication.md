...
`make up` starts three long-running development services: API, PostgreSQL and Mailpit. The migration container exits after setup. Compose forces password sign-in off and sends all OTP email to Mailpit, regardless of any production email key in `.env`. No SMTP port is exposed and no real email is sent. Open [the local inbox](http://localhost:8025), request a code from the app/API, and enter the captured code. Mailpit messages are disposable and disappear when the container is recreated.

For host Bun, start `docker compose up --detach --wait postgres mailpit`, set `AUTH_EMAIL_TRANSPORT=mailpit` and `MAILPIT_URL=http://localhost:8025` in the ignored `.env`, then follow `development.md`. Existing `.env` files are preserved by setup; the Compose overrides work without adding variables. Password auth remains an opt-in test utility only (`DEV_AUTH_ENABLED=true`), forbidden in production and disabled by the standard dev Compose service.
...
Resend delivery uses plain text and HTML, a branded sender, no OTP in the subject, a ten-second timeout per attempt and at most one transient retry with the same idempotency key. Verify your sending domain and publish the provider's SPF/DKIM records and your DMARC policy before live use. `mailpit` is forbidden under `NODE_ENV=production`. `disabled` leaves email sign-in explicitly unavailable. Keep production secrets in the hosting secret store. Changing `BETTER_AUTH_SECRET` invalidates outstanding OTP hashes as well as affecting signed auth material.
